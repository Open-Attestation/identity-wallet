...
import { fetchEncryptedDocument } from "./fetchDocument";

const mockfetchEncryptedDocument = fetchEncryptedDocument as jest.Mock;
jest.mock("./fetchDocument");
const dataPrefix = "https://action.openattestation.com?q=";

const EXPIRED_DOCUMENT_TTL = 1500000000;
...
describe("processQr", () => {
  ...
  it("should throw when processing expired documents", async () => {
    expect.assertions(1);
    jest.spyOn(Date, "now").mockImplementation(() => EXPIRED_DOCUMENT_TTL + 1);
    mockfetchEncryptedDocument.mockResolvedValue(demoExpired);
    const onDocumentStore = jest.fn();
    const onDocumentView = jest.fn();
  ...
  it("should not throw when processing non-expired documents", async () => {
    expect.assertions(2);
    jest.spyOn(Date, "now").mockImplementation(() => EXPIRED_DOCUMENT_TTL - 1);
    mockfetchEncryptedDocument.mockResolvedValue(demoExpired);
    const onDocumentStore = jest.fn();
    ...
    expect(onDocumentView).toHaveBeenCalledWith(demoExpired);
    expect(onDocumentStore).not.toHaveBeenCalled();
  });
});
